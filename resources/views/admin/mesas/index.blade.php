...
<div id="tableDetailModal" class="modal-overlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.7); backdrop-filter:blur(6px); z-index:9999; justify-content:center; align-items:center; padding:var(--sp-md); overflow-y:auto;">
    <div style="background:var(--clr-surface-1); border:1px solid var(--clr-border); border-radius:var(--radius-lg); width:100%; max-width:540px; padding:var(--sp-lg); box-shadow:0 20px 40px rgba(0,0,0,0.6); max-height:90vh; overflow-y:auto;">
        ...
        <div style="background:var(--clr-surface-2); border:1px solid var(--clr-border); border-radius:var(--radius-md); padding:var(--sp-md); margin-bottom:var(--sp-md); font-size:0.85rem;">
            ...
            <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                ...
                <strong id="modalTableShape" style="color:var(--clr-text);">Circular</strong>
            </div>
            <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
        ...
        <!-- ACTIVE CONSUMPTION / ORDER ITEMS SECTION (FOR OCCUPIED TABLES) -->
        <div id="modalActiveOrderContainer" style="display:none; background:var(--clr-surface-2); border:1px solid var(--clr-border); border-radius:var(--radius-md); padding:var(--sp-md); margin-bottom:var(--sp-md);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <h4 style="font-size:0.95rem; font-family:var(--font-display); color:var(--clr-primary); margin:0;">
                    🍽️ Comanda Activa en Mesa (<span id="activeOrderNumber">#ORD-101</span>)
                </h4>
                <strong id="activeOrderTotal" style="color:#34d399; font-size:1.1rem;">$0.00</strong>
            </div>
            <div id="activeOrderItemsList" style="display:flex; flex-direction:column; gap:6px; max-height:160px; overflow-y:auto; padding-right:4px;">
                <!-- Items injected dynamically -->
            </div>
        </div>

        <div style="display:flex; flex-direction:column; gap:var(--sp-xs); margin-bottom:var(--sp-md);">
            <!-- OCCUPIED TABLE EXCLUSIVE ACTIONS (CU-2.1.4 & CU-5.2.3) -->
            <div id="modalOccupiedActions" style="display:none; flex-direction:column; gap:var(--sp-xs);">
                <button onclick="openMoveMergeModal('mover')" class="btn btn-secondary" style="width:100%; display:flex; justify-content:center; align-items:center; gap:6px; border-color:var(--clr-primary); color:var(--clr-primary);">
                    🔀 Mover a otra Mesa
                </button>
                <button onclick="openMoveMergeModal('fusionar')" class="btn btn-secondary" style="width:100%; display:flex; justify-content:center; align-items:center; gap:6px; border-color:#a855f7; color:#c084fc;">
                    🔗 Fusionar Cuentas...
                </button>
                <button onclick="openSplitBillModal()" class="btn btn-primary" style="width:100%; display:flex; justify-content:center; align-items:center; gap:6px; background:#10b981; border:none; font-weight:700;">
                    ✂️ Dividir Cuenta (Split Bill) / Cobrar
                </button>
                <button onclick="handleLiberarMesaFromModal()" class="btn btn-danger" style="width:100%; display:flex; justify-content:center; align-items:center; gap:6px;">
                    🔓 Liberar / Desocupar Mesa Manualmente
                </button>
            </div>
...
<!-- ═══ CU-2.2.3 MODAL DE SEGURIDAD: ANULACIÓN DE PLATILLO CON PIN ═══ -->
<div id="anularItemModal" class="modal-overlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(8px); z-index:99999; justify-content:center; align-items:center; padding:var(--sp-md);">
    <div style="background:var(--clr-surface-1); border:1px solid #ef4444; border-radius:var(--radius-lg); width:100%; max-width:440px; padding:var(--sp-lg); box-shadow:0 20px 50px rgba(239,68,68,0.3);">
        <h3 style="font-family:var(--font-display); font-size:1.2rem; color:#f87171; margin-bottom:4px;">
            🔒 Anulación de Platillo Requiere PIN de Autorización
        </h3>
        <p style="font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:var(--sp-md);">
            Ingresa tu PIN de 4 dígitos de Gerencia o Capitán para autorizar la anulación de: <strong id="anularItemNameDisplay" style="color:#fff;">Platillo</strong>.
        </p>

        <form id="anularItemForm" onsubmit="handleConfirmAnularItem(event)">
            <input type="hidden" id="anularItemId" value="">
            
            <div style="margin-bottom:var(--sp-sm);">
                <label style="display:block; font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:4px;">PIN de Autorización (4 dígitos) *</label>
                <input type="password" id="anularPinInput" maxlength="6" required class="form-control" style="width:100%; background:var(--clr-surface-2); border:1px solid var(--clr-border); color:var(--clr-text); padding:10px 14px; border-radius:var(--radius-sm); font-size:1.2rem; text-align:center; letter-spacing:4px;" placeholder="****">
                <span style="font-size:0.75rem; color:var(--clr-text-muted); display:block; margin-top:2px;">(PINs predeterminados: 1234, 9999)</span>
            </div>

            <div style="margin-bottom:var(--sp-sm);">
                <label style="display:block; font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:4px;">Motivo de Anulación *</label>
                <select id="anularReasonSelect" required class="form-control" style="width:100%; background:var(--clr-surface-2); border:1px solid var(--clr-border); color:var(--clr-text); padding:8px 12px; border-radius:var(--radius-sm);">
                    <option value="Cliente cambió de opinión">🙋‍♂️ Cliente cambió de opinión</option>
                    <option value="Platillo defectuoso / mala calidad">🍲 Platillo defectuoso / mala calidad</option>
                    <option value="Error de mesero al tomar orden">📝 Error de mesero al tomar orden</option>
                    <option value="Demora excesiva en cocina">⏱️ Demora excesiva en cocina</option>
                    <option value="Otro motivo">📌 Otro motivo</option>
                </select>
            </div>

            <div style="margin-bottom:var(--sp-lg);">
                <label style="display:block; font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:4px;">Explicación / Notas Adicionales</label>
                <textarea id="anularNotesInput" class="form-control" style="width:100%; background:var(--clr-surface-2); border:1px solid var(--clr-border); color:var(--clr-text); padding:8px 12px; border-radius:var(--radius-sm); min-height:60px;" placeholder="Detalles de la anulación..."></textarea>
            </div>

            <div style="display:flex; justify-content:flex-end; gap:var(--sp-sm);">
                <button type="button" onclick="closeAnularItemModal()" class="btn btn-secondary">Cancelar</button>
                <button type="submit" class="btn btn-danger" style="background:#ef4444; font-weight:700;">Autorizar Anulación</button>
            </div>
        </form>
    </div>
</div>

<!-- ═══ CU-2.1.4 MODAL MOVER / FUSIONAR MESAS ═══ -->
<div id="moveMergeModal" class="modal-overlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(8px); z-index:99999; justify-content:center; align-items:center; padding:var(--sp-md);">
    <div style="background:var(--clr-surface-1); border:1px solid var(--clr-border); border-radius:var(--radius-lg); width:100%; max-width:460px; padding:var(--sp-lg); box-shadow:0 20px 50px rgba(0,0,0,0.8);">
        <h3 id="moveMergeModalTitle" style="font-family:var(--font-display); font-size:1.2rem; color:var(--clr-text); margin-bottom:4px;">
            🔀 Mover Mesa
        </h3>
        <p id="moveMergeModalSubtitle" style="font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:var(--sp-md);">
            Selecciona la mesa de destino a donde deseas transferir la comanda de la mesa actual.
        </p>

        <form id="moveMergeForm" onsubmit="handleConfirmMoveMerge(event)">
            <input type="hidden" id="moveMergeMode" value="mover">

            <div style="margin-bottom:var(--sp-lg);">
                <label style="display:block; font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:4px;">Mesa Destino *</label>
                <select id="moveMergeTargetSelect" required class="form-control" style="width:100%; background:var(--clr-surface-2); border:1px solid var(--clr-border); color:var(--clr-text); padding:10px 14px; border-radius:var(--radius-sm); font-size:1rem;"></select>
            </div>

            <div style="display:flex; justify-content:flex-end; gap:var(--sp-sm);">
                <button type="button" onclick="closeMoveMergeModal()" class="btn btn-secondary">Cancelar</button>
                <button type="submit" id="btnConfirmMoveMerge" class="btn btn-primary" style="background:var(--clr-primary); font-weight:600;">Confirmar Acción</button>
            </div>
        </form>
    </div>
</div>

<!-- ═══ CU-5.2.3 MODAL DIVISIÓN DE CUENTAS (SPLIT BILL) ═══ -->
<div id="splitBillModal" class="modal-overlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(8px); z-index:99999; justify-content:center; align-items:center; padding:var(--sp-md); overflow-y:auto;">
    <div style="background:var(--clr-surface-1); border:1px solid #10b981; border-radius:var(--radius-lg); width:100%; max-width:540px; padding:var(--sp-lg); box-shadow:0 20px 50px rgba(16,185,129,0.3); max-height:90vh; overflow-y:auto;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:var(--sp-md);">
            <h3 style="font-family:var(--font-display); font-size:1.3rem; color:#34d399; margin:0;">
                ✂️ Cobro & División de Cuentas (Split Bill)
            </h3>
            <button onclick="closeSplitBillModal()" style="background:none; border:none; color:var(--clr-text-muted); font-size:1.2rem; cursor:pointer;">✕</button>
        </div>

        <div style="background:var(--clr-surface-2); border:1px solid var(--clr-border); border-radius:var(--radius-md); padding:var(--sp-md); margin-bottom:var(--sp-md); display:flex; justify-content:space-between; align-items:center;">
            <div>
                <span style="font-size:0.85rem; color:var(--clr-text-muted);">Mesa: <strong id="splitBillTableName" style="color:#fff;">T1</strong></span><br>
                <span style="font-size:0.75rem; color:var(--clr-text-muted);">Comanda: <code id="splitBillOrderNum" style="color:var(--clr-primary);">#ORD-101</code></span>
            </div>
            <div style="text-align:right;">
                <span style="font-size:0.75rem; color:var(--clr-text-muted);">Total Comanda</span>
                <div id="splitBillTotalDisplay" style="font-size:1.4rem; font-weight:700; color:#34d399;">$0.00</div>
            </div>
        </div>

        <!-- Mode Toggle Switcher -->
        <div style="display:flex; background:var(--clr-surface-2); border-radius:var(--radius-md); padding:4px; margin-bottom:var(--sp-md);">
            <button type="button" id="btnModeEqual" onclick="switchSplitMode('equal')" class="btn btn-sm btn-primary" style="flex:1; border-radius:var(--radius-sm); font-size:0.85rem;">
                ⚖️ Por Montos Iguales
            </button>
            <button type="button" id="btnModeItems" onclick="switchSplitMode('items')" class="btn btn-sm btn-secondary" style="flex:1; border-radius:var(--radius-sm); font-size:0.85rem;">
                📦 Por Selección de Platillos
            </button>
        </div>

        <!-- Mode 1: Equal Amounts Form -->
        <div id="splitEqualSection">
            <div style="margin-bottom:var(--sp-md);">
                <label style="display:block; font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:4px;">Dividir total entre cuántas partes/personas?</label>
                <input type="number" id="splitPartsCount" min="1" max="20" value="2" onchange="renderEqualSplitParts()" onkeyup="renderEqualSplitParts()" class="form-control" style="width:100%; background:var(--clr-surface-2); border:1px solid var(--clr-border); color:var(--clr-text); padding:8px 12px; border-radius:var(--radius-sm);">
            </div>
            <div id="equalPartsContainer" style="display:flex; flex-direction:column; gap:10px; margin-bottom:var(--sp-lg);">
                <!-- Dynamic equal parts list -->
            </div>
        </div>

        <!-- Mode 2: Item Selection Form -->
        <div id="splitItemsSection" style="display:none;">
            <p style="font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:var(--sp-sm);">
                Cobro parcial seleccionando platillos específicos de la cuenta:
            </p>
            <div id="splitItemsList" style="display:flex; flex-direction:column; gap:8px; margin-bottom:var(--sp-md); max-height:200px; overflow-y:auto;">
                <!-- Product items checkbox list -->
            </div>
            <div style="background:var(--clr-surface-2); padding:10px; border-radius:8px; margin-bottom:var(--sp-md); display:flex; justify-content:space-between; align-items:center;">
                <span>Subtotal Seleccionado:</span>
                <strong id="splitItemsSubtotalDisplay" style="color:#34d399; font-size:1.1rem;">$0.00</strong>
            </div>
            <div style="margin-bottom:var(--sp-lg);">
                <label style="display:block; font-size:0.85rem; color:var(--clr-text-muted); margin-bottom:4px;">Método de Pago para esta parte</label>
                <select id="splitItemPaymentMethod" class="form-control" style="width:100%; background:var(--clr-surface-2); border:1px solid var(--clr-border); color:var(--clr-text); padding:8px 12px; border-radius:var(--radius-sm);">
                    <option value="1">💵 Efectivo en Caja</option>
                    <option value="2">💳 Tarjeta / Terminal Bancaria</option>
                    <option value="3">📲 Transferencia QR / CoDi</option>
                </select>
            </div>
        </div>

        <div style="display:flex; justify-content:flex-end; gap:var(--sp-sm);">
            <button type="button" onclick="closeSplitBillModal()" class="btn btn-secondary">Cancelar</button>
            <button type="button" onclick="handleConfirmSplitBill()" class="btn btn-success" style="background:#10b981; font-weight:700;">
                💳 Procesar Cobro Split
            </button>
        </div>
    </div>
</div>

<!-- ═══ SUB-MODAL GESTIÓN QR ═══ -->
...
<div id="areaModal" class="modal-overlay" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.7); backdrop-filter:blur(4px); z-index:9999; justify-content:center; align-items:center; padding:var(--sp-md);">
    <div style="background:var(--clr-surface-1); border:1px solid var(--clr-border); border-radius:var(--radius-lg); width:100%; max-width:500px; padding:var(--sp-lg); box-shadow:0 20px 40px rgba(0,0,0,0.5);">
        ...
        <form id="areaForm" onsubmit="handleSaveArea(event)">
            ...
            <div style="display:flex; flex-direction:row; align-items:center; gap:var(--sp-lg); margin-bottom:var(--sp-md); flex-wrap:wrap;">
            ...
            <!-- Interactive Tables List inside Area Edit Modal -->
            <div id="areaTablesSection" style="display:none; margin-bottom:var(--sp-lg); padding-top:var(--sp-md); border-top:1px solid var(--clr-border);">
                <h4 style="font-size:0.95rem; font-family:var(--font-display); color:var(--clr-primary); margin-bottom:8px; display:flex; align-items:center; gap:6px;">
                    🪑 Mesas Asignadas en este Salón (<span id="areaTablesCount">0</span>)
                </h4>
                <div id="areaTablesList" style="display:flex; flex-direction:column; gap:6px; max-height:180px; overflow-y:auto; padding-right:4px;">
                    <!-- Tables list injected dynamically -->
                </div>
            </div>

            <div style="display:flex; justify-content:flex-end; gap:var(--sp-sm);">
                <button type="button" onclick="closeAreaModal()" class="btn btn-secondary">Cancelar</button>
                <button type="submit" id="btnSaveArea" class="btn btn-primary" style="background:var(--clr-primary); color:#fff; font-weight:600;">Guardar Área</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div style="background:var(--clr-surface-1); border:1px solid var(--clr-border); border-radius:var(--radius-lg); width:100%; max-width:480px; padding:var(--sp-lg); box-shadow:0 20px 40px rgba(0,0,0,0.5);">
        ...
        <form id="singleMesaForm" onsubmit="handleSaveSingleMesa(event)">
            ...
            <div style="display:flex; justify-content:flex-end; gap:var(--sp-sm);">
                ...
                <button type="submit" id="btnSaveSingleMesa" class="btn btn-primary" style="background:var(--clr-primary); color:#fff; font-weight:600;">Guardar Mesa</button>
            </div>
        </form>
    </div>
</div>
        ...
        <div style="background:var(--clr-surface-1); border:1px solid ${area.is_active ? 'var(--clr-border)' : '#f43f5e'}; border-radius:var(--radius-lg); padding:var(--sp-lg); opacity: ${area.is_active ? '1' : '0.7'}; box-shadow:0 10px 30px rgba(0,0,0,0.3);">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:var(--sp-sm); margin-bottom:var(--sp-md);">
                ...
                <div style="display:flex; align-items:center; gap:var(--sp-sm); flex-wrap:wrap;">
                    ...
                    <button onclick="openTableModal(${area.id})" ${isLimitReached || !area.is_active ? 'disabled' : ''} class="btn btn-sm btn-primary">
                        ⚡ Gen. Masiva
                    </button>
            ...
            <div class="floor-plan-canvas">
                ${tables.length === 0 ? `
                    <div style="width:100%; text-align:center; padding:var(--sp-xl); color:var(--clr-text-muted);">
                        Lienzo vacío. Haz clic en <strong>⚡ Gen. Masiva</strong> para añadir mesas.
                    </div>
...
function handleTableClick(tableJson, areaName) {
    ...
    document.getElementById('modalTableShape').innerText = table.shape === 'rect' ? '🟦 Rectangular' : '🟢 Circular';
    ...
    const occupiedActions = document.getElementById('modalOccupiedActions');
    const activeOrderContainer = document.getElementById('modalActiveOrderContainer');
    ...
    if (!table.is_active) {
        ...
        occupiedActions.style.display = 'none';
    } else if (table.status === 'occupied') {
        badge.className = 'badge badge-danger';
        badge.innerText = '🔴 Ocupada';
        statusText.innerText = 'Comensales activos en mesa con consumo';
        toggleBtn.innerText = '🔴 Desactivar Mesa';
        toggleBtn.className = 'btn btn-secondary';
        occupiedActions.style.display = 'flex'; // Show Move, Merge, Split buttons

        // Render Active Order details & Item Cancellation options
        const activeOrders = table.active_orders || table.activeOrders || [];
        if (activeOrders.length > 0) {
            activeOrderData = activeOrders[0];
            document.getElementById('activeOrderNumber').innerText = activeOrderData.order_number;
            document.getElementById('activeOrderTotal').innerText = `$${parseFloat(activeOrderData.total).toFixed(2)}`;

            const itemsList = document.getElementById('activeOrderItemsList');
            const items = activeOrderData.items || [];
            if (items.length > 0) {
                itemsList.innerHTML = items.map(item => `
                    <div style="display:flex; justify-content:space-between; align-items:center; background:var(--clr-surface-1); padding:6px 10px; border-radius:6px; font-size:0.82rem;">
                        <div>
                            <strong>${item.quantity}x ${escapeHtml(item.product_name)}</strong>
                            <span style="color:var(--clr-text-muted); font-size:0.75rem;">($${parseFloat(item.unit_price).toFixed(2)})</span>
                        </div>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <strong style="color:var(--clr-text);">$${parseFloat(item.total).toFixed(2)}</strong>
                            <button onclick="openAnularItemModal(${item.id}, '${escapeHtml(item.product_name)}')" class="btn btn-sm btn-danger" style="padding:2px 6px; font-size:0.7rem;" title="Anular Platillo (PIN)">
                                🗑️ Anular
                            </button>
                        </div>
                    </div>
                ...
                itemsList.innerHTML = `<span style="font-size:0.8rem; color:var(--clr-text-muted);">Sin platillos en comanda</span>`;
            }
...
/* ═════════════════════════════════════════════════════════════════════════════
   CU-2.2.3: ANULACIÓN DE PLATILLOS CON PIN DE SEGURIDAD
   ═════════════════════════════════════════════════════════════════════════════ */
function openAnularItemModal(itemId, productName) {
    document.getElementById('anularItemId').value = itemId;
    document.getElementById('anularItemNameDisplay').innerText = productName;
    document.getElementById('anularPinInput').value = '';
    document.getElementById('anularNotesInput').value = '';
    document.getElementById('anularItemModal').style.display = 'flex';
}

function closeAnularItemModal() {
    document.getElementById('anularItemModal').style.display = 'none';
}

async function handleConfirmAnularItem(e) {
    e.preventDefault();
    const itemId = parseInt(document.getElementById('anularItemId').value);
    const pin = document.getElementById('anularPinInput').value.trim();
    const reason = document.getElementById('anularReasonSelect').value;
    const notes = document.getElementById('anularNotesInput').value.trim();

    try {
        const res = await fetch('/api/admin/comandas/anular-item', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        const data = await res.json();
        if (!res.ok) {
            showToast(data.message || 'PIN incorrecto o error al anular', 'error');
            return;
        }

        showToast(data.message);
        closeAnularItemModal();
        closeTableDetailModal();
        loadAreas();
    } catch (err) {
        showToast('Error al conectar con el servidor', 'error');
...
/* ═════════════════════════════════════════════════════════════════════════════
   ...
   ═════════════════════════════════════════════════════════════════════════════ */
function openMoveMergeModal(mode) {
    if (!activeSelectedTable) return;
    document.getElementById('moveMergeMode').value = mode;

    const title = mode === 'mover' ? '🔀 Mover Comanda a Otra Mesa' : '🔗 Fusionar Cuentas de Mesas';
    const sub = mode === 'mover' ? 'Selecciona la mesa libre a la cual transferir los platillos.' : 'Selecciona la mesa ocupada con la cual unir la comanda actual.';

    document.getElementById('moveMergeModalTitle').innerText = title;
    document.getElementById('moveMergeModalSubtitle').innerText = sub;

    // Populate target tables (excluding current table)
    const select = document.getElementById('moveMergeTargetSelect');
    let targetOptions = [];

    areasData.forEach(area => {
        (area.tables || []).forEach(t => {
            if (t.id !== activeSelectedTable.id && t.is_active) {
                if (mode === 'mover' && t.status !== 'occupied') {
                    targetOptions.push(`<option value="${t.id}">🟢 ${escapeHtml(t.name || t.table_code)} (${escapeHtml(area.name)})</option>`);
                } else if (mode === 'fusionar') {
                    const icon = t.status === 'occupied' ? '🔴' : '🟢';
                    targetOptions.push(`<option value="${t.id}">${icon} ${escapeHtml(t.name || t.table_code)} (${escapeHtml(area.name)})</option>`);
                }
            }
        });
    });
    ...
    if (targetOptions.length === 0) {
        select.innerHTML = `<option value="">No hay mesas disponibles para ${mode}</option>`;
        document.getElementById('btnConfirmMoveMerge').disabled = true;
    } else {
        select.innerHTML = targetOptions.join('');
        document.getElementById('btnConfirmMoveMerge').disabled = false;
    }

    document.getElementById('moveMergeModal').style.display = 'flex';
}

function closeMoveMergeModal() {
    document.getElementById('moveMergeModal').style.display = 'none';
}

async function handleConfirmMoveMerge(e) {
    e.preventDefault();
    const mode = document.getElementById('moveMergeMode').value;
    const targetTableId = parseInt(document.getElementById('moveMergeTargetSelect').value);

    if (!targetTableId) {
        showToast('Selecciona una mesa destino válida', 'error');
        return;
    }

    const endpoint = mode === 'mover' ? '/api/admin/mesas/mover' : '/api/admin/mesas/fusionar';
    ...
    try {
        const res = await fetch(endpoint, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                source_table_id: activeSelectedTable.id,
...
/* ═════════════════════════════════════════════════════════════════════════════
   CU-5.2.3: DIVISIÓN DE CUENTAS (SPLIT BILL)
   ═════════════════════════════════════════════════════════════════════════════ */
function openSplitBillModal() {
    if (!activeSelectedTable || !activeOrderData) {
        showToast('No hay una comanda activa disponible para cobrar', 'error');
        return;
    }

    document.getElementById('splitBillTableName').innerText = activeSelectedTable.name || activeSelectedTable.table_code;
    document.getElementById('splitBillOrderNum').innerText = activeOrderData.order_number;
    document.getElementById('splitBillTotalDisplay').innerText = `$${parseFloat(activeOrderData.total).toFixed(2)}`;
...
function switchSplitMode(mode) {
    ...
    const btnEqual = document.getElementById('btnModeEqual');
    const btnItems = document.getElementById('btnModeItems');
    const secEqual = document.getElementById('splitEqualSection');
    const secItems = document.getElementById('splitItemsSection');

    if (mode === 'equal') {
        btnEqual.className = 'btn btn-sm btn-primary';
        btnItems.className = 'btn btn-sm btn-secondary';
        secEqual.style.display = 'block';
        secItems.style.display = 'none';
        renderEqualSplitParts();
    } else {
        btnEqual.className = 'btn btn-sm btn-secondary';
        btnItems.className = 'btn btn-sm btn-primary';
        secEqual.style.display = 'none';
        secItems.style.display = 'block';
        renderItemsSplitList();
    }
}

function renderEqualSplitParts() {
    const partsCount = parseInt(document.getElementById('splitPartsCount').value) || 1;
    const totalOrder = parseFloat(activeOrderData.total) || 0;
    const partAmount = (totalOrder / partsCount).toFixed(2);

    const container = document.getElementById('equalPartsContainer');
    let html = '';

    for (let i = 1; i <= partsCount; i++) {
        html += `
        <div style="background:var(--clr-surface-2); padding:10px 14px; border-radius:8px; border:1px solid var(--clr-border); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
            <div>
                <strong>Persona ${i}</strong>
                <div style="font-size:0.8rem; color:var(--clr-text-muted);">Cuota parte: <span style="color:#34d399; font-weight:700;">$${partAmount}</span></div>
            </div>
            <div style="display:flex; align-items:center; gap:8px;">
                <select id="splitPayMethod_${i}" class="form-control" style="background:var(--clr-surface-1); border:1px solid var(--clr-border); color:var(--clr-text); padding:4px 8px; border-radius:4px; font-size:0.8rem;">
...
function renderItemsSplitList() {
    const container = document.getElementById('splitItemsList');
    const items = activeOrderData.items || [];

    if (items.length === 0) {
        container.innerHTML = `<span style="font-size:0.85rem; color:var(--clr-text-muted);">No hay platillos en la comanda</span>`;
        return;
    }

    container.innerHTML = items.map(item => `
        <label style="background:var(--clr-surface-2); padding:10px; border-radius:6px; border:1px solid var(--clr-border); display:flex; justify-content:space-between; align-items:center; cursor:pointer;">
            <div style="display:flex; align-items:center; gap:10px;">
                <input type="checkbox" class="split-item-checkbox" value="${item.id}" data-total="${item.total}" onchange="recalcItemsSplitTotal()" style="width:18px; height:18px; accent-color:var(--clr-primary);">
                <span>${item.quantity}x <strong>${escapeHtml(item.product_name)}</strong></span>
            </div>
            <strong style="color:#34d399;">$${parseFloat(item.total).toFixed(2)}</strong>
        </label>
    `).join('');

    recalcItemsSplitTotal();
}

function recalcItemsSplitTotal() {
    const checkboxes = document.querySelectorAll('.split-item-checkbox:checked');
    let subtotal = 0;
    checkboxes.forEach(cb => {
        subtotal += parseFloat(cb.getAttribute('data-total')) || 0;
    });
    document.getElementById('splitItemsSubtotalDisplay').innerText = `$${subtotal.toFixed(2)}`;
}

async function handleConfirmSplitBill() {
    let paymentsPayload = [];

    if (currentSplitMode === 'equal') {
        const partsCount = parseInt(document.getElementById('splitPartsCount').value) || 1;
        const totalOrder = parseFloat(activeOrderData.total) || 0;
        const partAmount = (totalOrder / partsCount);

        for (let i = 1; i <= partsCount; i++) {
            const methodId = parseInt(document.getElementById(`splitPayMethod_${i}`).value) || 1;
        ...
        if (checkboxes.length === 0) {
            showToast('Selecciona al menos un platillo para realizar el cobro parcial', 'error');
            return;
        }

        let selectedTotal = 0;
        checkboxes.forEach(cb => {
            selectedTotal += parseFloat(cb.getAttribute('data-total')) || 0;
        });

        const methodId = parseInt(document.getElementById('splitItemPaymentMethod').value) || 1;
        paymentsPayload.push({
    ...
    try {
        const res = await fetch('/api/admin/mesas/cobrar-split', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        const data = await res.json();
        if (!res.ok) {
            showToast(data.message || 'Error al procesar el cobro', 'error');
            return;
        }
...
function openAreaModal(id = null) {
    ...
    document.getElementById('areaModalTitle').innerText = id ? 'Editar Área / Salón' : 'Nueva Área de Servicio';
    ...
    if (id) {
        ...
        renderAreaTablesInModal(id);
    } else {
        document.getElementById('areaTablesSection').style.display = 'none';
    }

    document.getElementById('areaModal').style.display = 'flex';
}

function renderAreaTablesInModal(areaId) {
    const areaTablesSection = document.getElementById('areaTablesSection');
    const areaTablesList = document.getElementById('areaTablesList');
    const areaTablesCount = document.getElementById('areaTablesCount');

    if (!areaId) {
        areaTablesSection.style.display = 'none';
        return;
    }

    const area = areasData.find(a => a.id === parseInt(areaId));
    const tables = (area && area.tables) ? area.tables : [];

    areaTablesCount.innerText = tables.length;
    areaTablesSection.style.display = 'block';

    if (tables.length === 0) {
        areaTablesList.innerHTML = `<span style="font-size:0.82rem; color:var(--clr-text-muted);">No hay mesas asignadas a este salón.</span>`;
        return;
    }

    areaTablesList.innerHTML = tables.map(t => {
        let statusBadge = '<span class="badge badge-success" style="font-size:0.75rem;">🟢 Libre</span>';
        if (!t.is_active) statusBadge = '<span class="badge badge-danger" style="font-size:0.75rem;">⚪ Inactiva</span>';
        else if (t.status === 'occupied') statusBadge = '<span class="badge badge-danger" style="font-size:0.75rem;">🔴 Ocupada</span>';
        else if (t.status === 'reserved') statusBadge = '<span class="badge badge-warning" style="font-size:0.75rem;">🟡 Reservada</span>';

        return `
        <div style="display:flex; justify-content:space-between; align-items:center; background:var(--clr-surface-2); padding:8px 12px; border-radius:6px; border:1px solid var(--clr-border);">
            <div>
                <strong style="color:var(--clr-text); font-size:0.88rem;">${escapeHtml(t.name || t.table_code)}</strong>
                <span style="font-size:0.78rem; color:var(--clr-text-muted); margin-left:6px;">(👤 ${t.capacity} Sillas)</span>
            </div>
            <div style="display:flex; align-items:center; gap:8px;">
                ${statusBadge}
                <button type="button" onclick="handleDeleteMesaFromAreaModal(${t.id}, '${escapeHtml(t.name || t.table_code)}', ${area.id})" class="btn btn-sm btn-danger" style="padding:3px 8px; font-size:0.75rem; background:#dc2626; border-color:#dc2626;" title="Eliminar Mesa del Salón">
                    🗑️ Eliminar
                </button>
            </div>
        </div>
...
async function handleDeleteMesaFromAreaModal(tableId, tableName, areaId) {
    if (!confirm(`¿Estás seguro de eliminar la mesa "${tableName}" de este salón?`)) {
        return;
    }

    try {
        const res = await fetch(`/api/admin/mesas/${tableId}`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' }
        });
        ...
        const data = await res.json();
        if (!res.ok) {
            showToast(data.message || 'Error al eliminar la mesa', 'error');
            return;
        }

        showToast(data.message);
        await loadAreas(); // Live update floor plan and counters
        renderAreaTablesInModal(areaId); // Live update modal list
    } catch (err) {
        showToast('Error al conectar con el servidor', 'error');
    }
}
